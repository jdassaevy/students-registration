(function (root) {
    ...
    function resolveConsentTimestamp(
        previousConsent,
        nextConsent,
        previousTimestamp,
        ...
        if (!nextConsent) 
            return null;
        if (previousConsent && previousTimestamp) 
    ...
    function injectStyles() {
        if (byId('academyAutomationStyles')) 
            return;
    ...
    function contactMarkup(person) {
        const label = person === 'person1'
            ? 'primeira pessoa'
            : 'segunda pessoa';
        const prefix = person === 'person1'
            ? 'p1'
            : 'p2';
        return `
    ...
    function injectStudentContactFields() {
        if (byId('p1Phone')) 
            return;
        const p1Block = byId('p1EntryValue')
            ?.closest('.financial-person-block');
        const p2Block = byId('p2EntryValue')
            ?.closest('.financial-person-block');
        if (p1Block) 
            p1Block.insertAdjacentHTML('beforeend', contactMarkup('person1'));
        if (p2Block) 
            p2Block.insertAdjacentHTML('beforeend', contactMarkup('person2'));
        [
            [
                'p1Phone', 'p1WhatsappConsent'
            ],
            [
                'p2Phone', 'p2WhatsappConsent'
            ]
        ].forEach(([phoneId, consentId]) => {
    ...
    async function loadAcademyProfile() {
        ...
        byId('academyName').value = data
            ?.academy_name || '';
        byId('academyResponsible').value = data
            ?.responsible_name || '';
        byId('academySupportPhone').value = data
            ?.support_phone || '';
        byId('academyDisplayName').value = data
            ?.display_name || '';
    }
    ...
    async function saveAcademyProfile(event) {
        ...
        if (typeof currentUser === 'undefined' || !currentUser) 
            return;
        const supportRaw = byId('academySupportPhone')
            .value
            .trim();
        const supportPhone = normalizePhone(supportRaw);
        if (supportRaw && !supportPhone) {
            if (typeof toast === 'function') 
                toast('Informe um telefone válido para dúvidas.');
            return;
        }
        const academyName = byId('academyName')
            .value
            .trim();
        const payload = {
            user_id: currentUser.id,
            academy_name: academyName,
            responsible_name: byId('academyResponsible')
                .value
                .trim(),
            support_phone: supportPhone,
            display_name: byId('academyDisplayName')
                .value
                .trim() || null,
        ...
        closeSettingsDialog();
        if (typeof toast === 'function') 
            toast('Configurações da academia salvas!');
        document.dispatchEvent(
            new CustomEvent('academy-profile-updated', {detail: payload})
        );
    }

    function clearContactFields() {
        ['p1Phone', 'p2Phone'].forEach(id => {
            if (byId(id)) 
                byId(id).value = '';
            }
        );
    ...
    async function populateStudentContactFields(id) {
        ...
        const {data, error} = await db
            .from('students')
            .select(
                'person1_phone, person2_phone, person1_whatsapp_consent, person2_whatsapp_conse' +
                'nt'
            )
            .eq('id', id)
            .single();
        ...
        byId('p1WhatsappConsent').checked = Boolean(
            data.person1_phone && data.person1_whatsapp_consent
        );
        byId('p2WhatsappConsent').checked = Boolean(
            data.person2_phone && data.person2_whatsapp_consent
        );
    ...
    async function saveStudentWithAutomationData(event) {
        ...
        const person2 = byId('person2')
            .value
            .trim();
        const p1Raw = byId('p1Phone')
            .value
            .trim();
        const p2Raw = byId('p2Phone')
            .value
            .trim();
        const p1Phone = normalizePhone(p1Raw);
        const p2Phone = person2
            ? normalizePhone(p2Raw)
            : null;
        ...
        const previous = id
            ? await db
                .from('students')
                .select(
                    'person1_whatsapp_consent, person2_whatsapp_consent, person1_whatsapp_consent_a' +
                    't, person2_whatsapp_consent_at'
                )
                .eq('id', id)
                .single()
            : {
        ...
        const p2Consent = Boolean(
            person2 && p2Phone && byId('p2WhatsappConsent').checked
        );
        const nowIso = new Date().toISOString();
        const old = previous.data || {};

        const payload = {
            person1: byId('person1')
                .value
                .trim(),
            person2: person2 || null,
            class_id: byId('coupleClass').value || null,
            entry_paid: byId('p1Entry').checked || Boolean(
                person2 && byId('p2Entry').checked
            ),
            entry_payments: {
                person1: byId('p1Entry').checked,
                person2: person2
                    ? byId('p2Entry').checked
                    : false
            },
            fees: {
                person1: {
                    entry: inputMoney('p1EntryValue'),
                    monthly: inputMoney('p1MonthlyValue')
                },
                person2: person2
                    ? {
                        entry: inputMoney('p2EntryValue'),
                        monthly: inputMoney('p2MonthlyValue')
                    }
                    : {
                        entry: 0,
                        monthly: 0
            ...
            payments: {
                ...
                person2: person2
                    ? [1, 2, 3].map(i => byId(`p2m${i}`).checked)
                    : [false, false, false]
            },
            ...
            person1_whatsapp_consent_at: resolveConsentTimestamp(
                Boolean(old.person1_whatsapp_consent),
                p1Consent,
                old.person1_whatsapp_consent_at,
                nowIso
            ),
            person2_whatsapp_consent_at: resolveConsentTimestamp(
                Boolean(old.person2_whatsapp_consent),
                p2Consent,
                old.person2_whatsapp_consent_at,
                nowIso
            )
        };
    ...
    function wireUi() {
        ...
        byId('academySettingsBtn')
            ?.addEventListener('click', async () => {
                await loadAcademyProfile();
                openSettingsDialog();
            });
        byId('academySettingsForm')
            ?.addEventListener('submit', saveAcademyProfile);
        byId('closeAcademySettings')
            ?.addEventListener('click', closeSettingsDialog);
        byId('cancelAcademySettings')
            ?.addEventListener('click', closeSettingsDialog);
        byId('academySettingsModal')
        ...
        byId('newBtn')
            ?.addEventListener('click', clearContactFields);
        ...
        byId('form')
            ?.addEventListener('submit', saveStudentWithAutomationData, true);
    }
