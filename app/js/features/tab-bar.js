(() => {
    ...
    const TAB_META = {
        dashboardTab: {
            label: 'Visão Geral',
            icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h6v6h-6z"/></svg>'
        },
        studentsTab: {
            label: 'Alunos',
        ...
        reportsTab: {
            label: 'Relatórios',
            icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20h16v2H2V4h2v16Zm3-2H5v-6h2v6Zm5 0H9V8h3v10Zm5 0h-3V3h3v15Zm4 0h-2v-9h2v9Z"/></svg>'
        },
        automationTab: {
            label: 'Automações',
            icon: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.2 2 5 13h5.7L9.8 22 19 10h-5.8l0-8Z"/></svg>'
        }
    };

    function createAutomationFallback() {
        if (document.getElementById('automationTab')) return document.getElementById('automationTab');
        ...
        tab.dataset.automationFallback = 'true';
        nav.appendChild(tab);

        tab.addEventListener('click', () => {
            if (document.getElementById('automationView') && typeof setView === 'function') {
                setView('automation');
                return;
            }

            if (tab.dataset.loadingAutomation === 'true') return;
            tab.dataset.loadingAutomation = 'true';
            const fallbackTab = tab;
            fallbackTab.remove();

            const script = document.createElement('script');
            script.src = './js/features/automation-center.js?v=3';
            ...
            script.onerror = () => createAutomationFallback();
    ...
    function decorateTab(tab) {
        ...
        if (!meta || tab.dataset.iconTab === 'true') return;
