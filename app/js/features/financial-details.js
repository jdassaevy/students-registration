(function (root) {
  function personFinancialSummary(couple, person) {
    ...
    const fees = couple.fees?.[person] || {entry: 0, monthly: 0};
    const entryPaid = Boolean(couple.entryPayments?.[person]);
    const months = Array.isArray(couple.payments?.[person]) ? couple.payments[person].slice(0, 3).map(Boolean) : [false, false, false];
    const paidMonths = months.filter(Boolean).length;
    const received = (entryPaid ? Number(fees.entry || 0) : 0) + (paidMonths * Number(fees.monthly || 0));
    const pending = (!entryPaid ? Number(fees.entry || 0) : 0) + ((3 - paidMonths) * Number(fees.monthly || 0));
    return {entryPaid, months, paidMonths, paidCount: Number(entryPaid) + paidMonths, received, pending};
  }

  function groupFinancialItems(couplesList, classesList, filter) {
    const items = couplesList.filter(c => filter === 'all' || (filter === 'none' ? !c.classId : c.classId === filter));
    ...
    return [...groups.values()].map(group => {
      let students = 0, entries = 0, monthly = 0, received = 0;
      group.items.forEach(c => {
        ['person1', 'person2'].forEach(person => {
          const summary = personFinancialSummary(c, person);
          if (!summary) return;
          students += 1;
          const fees = c.fees?.[person] || {entry: 0, monthly: 0};
          entries += summary.entryPaid ? Number(fees.entry || 0) : 0;
          monthly += summary.paidMonths * Number(fees.monthly || 0);
          received += summary.received;
        });
      });
      const classItem = classesList.find(item => item.id === group.id);
      return {...group, students, entries, monthly, received, name: classItem ? classItem.name : 'Sem turma'};
    });
  }

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = {personFinancialSummary, groupFinancialItems};
  }

  if (!root || !root.document) return;
  const document = root.document;
  const escape = value => typeof escapeHtml === 'function' ? escapeHtml(value) : String(value ?? '').replace(/[&<>\"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[c]));
  const formatMoney = value => typeof money === 'function' ? money(value) : Number(value || 0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});
  ...
  document.getElementById('appView').appendChild(modal);
  ...
  function renderPerson(c, person, name) {
    ...
    return `<div class="financial-person-detail">
      <div class="financial-person-name"><strong>${escape(name)}</strong><small>Mensalidade: ${formatMoney(fees.monthly)}</small></div>
      <div><span class="financial-entry-status ${summary.entryPaid ? 'ok' : 'bad'}">Inscrição ${summary.entryPaid ? 'paga' : 'pendente'}</span><small style="display:block;color:var(--muted);margin-top:3px">${formatMoney(fees.entry)}</small></div>
      <div class="financial-month-statuses">${summary.months.map((on,i)=>`<span class="financial-month-chip ${on ? 'ok' : 'bad'}">${i+1}ª ${on ? '✓' : '—'}</span>`).join('')}</div>
      <div class="financial-person-received"><small>Recebido</small><strong>${formatMoney(summary.received)}</strong></div>
    </div>`;
  }

  function openGroup(groupId) {
    const currentCouples = typeof couples !== 'undefined' ? couples : [];
    const currentClasses = typeof classes !== 'undefined' ? classes : [];
    const filterItems = currentCouples.filter(c => (c.classId || 'none') === groupId);
    const classItem = currentClasses.find(item => item.id === groupId);
    const name = classItem ? classItem.name : 'Sem turma';
    const groups = groupFinancialItems(filterItems, currentClasses, 'all');
    const group = groups[0];
    if (!group) return;
    document.getElementById('financialDetailTitle').textContent = name;
    document.getElementById('financialDetailSubtitle').textContent = `${group.students} aluno${group.students === 1 ? '' : 's'} • detalhes de inscrições e mensalidades`;
    const pending = group.items.reduce((total,c)=> total + ['person1','person2'].reduce((sum,p)=>sum + (personFinancialSummary(c,p)?.pending || 0),0),0);
    document.getElementById('financialDetailContent').innerHTML = `<div class="financial-detail-summary"><div><span>Total recebido</span><strong>${formatMoney(group.received)}</strong></div><div><span>Inscrições</span><strong>${formatMoney(group.entries)}</strong></div><div><span>Valor pendente</span><strong>${formatMoney(pending)}</strong></div></div><div class="financial-student-list">${group.items.map(c=>{
      const total = ['person1','person2'].reduce((sum,p)=>sum+(personFinancialSummary(c,p)?.received || 0),0);
      return `<article class="financial-student-card"><div class="financial-student-head"><div><strong>${escape(c.person1)}${c.person2 ? ` &amp; ${escape(c.person2)}` : ''}</strong><small>${c.person2 ? 'Casal' : 'Aluno individual'}</small></div><span class="financial-student-total">${formatMoney(total)}</span></div>${renderPerson(c,'person1',c.person1)}${c.person2 ? renderPerson(c,'person2',c.person2) : ''}</article>`;
    }).join('')}</div>`;
    if (typeof openDialog === 'function') openDialog(modal); else modal.showModal();
  }

  const originalRenderFinancial = typeof renderFinancial === 'function' ? renderFinancial : null;
  renderFinancial = function () {
    if (typeof originalRenderFinancial === 'function') originalRenderFinancial();
    const select = document.getElementById('financialClassFilter');
    const list = document.getElementById('financialList');
    if (!select || !list) return;
    const currentCouples = typeof couples !== 'undefined' ? couples : [];
    const currentClasses = typeof classes !== 'undefined' ? classes : [];
    const groups = groupFinancialItems(currentCouples, currentClasses, select.value);
    if (!groups.length) return;
    list.innerHTML = groups.map(group => `<tr class="financial-row-clickable" data-financial-group="${escape(group.id)}" tabindex="0" role="button" aria-label="Ver detalhes financeiros de ${escape(group.name)}"><td><div class="financial-row-name"><span class="class-name">${escape(group.name)}</span><span class="financial-row-arrow">›</span></div></td><td>${group.students}</td><td>${formatMoney(group.entries)}</td><td>${formatMoney(group.monthly)}</td><td class="financial-total-cell">${formatMoney(group.received)}</td></tr>`).join('');
  };

  const list = document.getElementById('financialList');
  list?.addEventListener('click', event => {
    const row = event.target.closest('[data-financial-group]');
    if (row) openGroup(row.dataset.financialGroup);
  });
  ...
  document.getElementById('closeFinancialDetail').onclick = () => typeof closeDialog === 'function' ? closeDialog(modal) : modal.close();
  modal.addEventListener('cancel', event => {event.preventDefault(); typeof closeDialog === 'function' ? closeDialog(modal) : modal.close();});
