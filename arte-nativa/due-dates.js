(function (root) {
  ...
  function addMonthsClamped(start, months) {
    const source = start instanceof Date ? new Date(start.getFullYear(), start.getMonth(), start.getDate()) : parseDateOnly(start);
    if (!source) return null;
    const targetMonth = source.getMonth() + months;
    const lastDay = new Date(source.getFullYear(), targetMonth + 1, 0).getDate();
    return new Date(source.getFullYear(), targetMonth, Math.min(source.getDate(), lastDay));
  }

  function calculateMonthlyDueDates(startDate, count = 3) {
    const start = parseDateOnly(startDate);
    if (!start) return [];
    return Array.from({length: count}, (_, index) => toDateOnly(addMonthsClamped(start, index)));
  }

  function formatDate(value) {
    const date = parseDateOnly(value);
    return date ? new Intl.DateTimeFormat('pt-BR').format(date) : 'Não definido';
  }

  if (typeof module !== 'undefined' && module.exports) {
    module.exports = {parseDateOnly, addMonthsClamped, calculateMonthlyDueDates, formatDate};
  }
  ...
  if (classSchedule) {
    ...
    classSchedule.closest('.field')?.insertAdjacentElement('afterend', startField);
  }

  async function loadClassStarts() {
    try {
      const {data, error} = await db.from('classes').select('id,start_date');
      if (error) throw error;
      starts.clear();
      (data || []).forEach(item => {
        if (item.start_date) starts.set(item.id, item.start_date);
      });
      decorateClassList();
    } catch (error) {
  ...
  function decorateClassList() {
    ...
    items.forEach((element, index) => {
      element.querySelector('.class-due-info')?.remove();
      const classItem = typeof classes !== 'undefined' ? classes[index] : null;
      if (!classItem) return;
      ...
      info.textContent = `Início: ${formatDate(start)} • Vencimentos: ${due.map(formatDate).join(' • ')}`;
      element.querySelector('div')?.appendChild(info);
    });
  }

  const originalRenderClassList = typeof renderClassList === 'function' ? renderClassList : null;
  ...
  classForm?.addEventListener('submit', async event => {
    ...
    if (!startDate) {
      if (typeof toast === 'function') toast('Defina a data de início da turma.');
      return;
    }

    const payload = {
      name: document.getElementById('className').value.trim(),
      place: document.getElementById('classPlace').value.trim(),
      schedule: document.getElementById('classSchedule').value.trim(),
      start_date: startDate
    };

    const {data, error} = await db.from('classes').insert(payload).select().single();
    if (error) {
      if (typeof toast === 'function') toast('Não foi possível criar a turma.');
      return;
    }

    const mapped = typeof fromClass === 'function' ? fromClass(data) : {
      id: data.id,
      name: data.name,
      place: data.place || '',
    ...
    if (typeof toast === 'function') toast('Turma criada com vencimentos definidos!');
  }, true);

  function decorateFinancialDetails() {
    const content = document.getElementById('financialDetailContent');
    if (!content || !activeFinancialClassId) return;
    content.querySelector('.financial-due-banner')?.remove();
    ...
    banner.innerHTML = `<strong>Início da turma:</strong> ${formatDate(start)} &nbsp;•&nbsp; <strong>Vencimentos:</strong> ${dueDates.map((date, index) => `${index + 1}ª ${formatDate(date)}`).join(' • ')}`;
    ...
    [...content.querySelectorAll('.financial-month-statuses')].forEach(group => {
      [...group.querySelectorAll('.financial-month-chip')].forEach((chip, index) => {
        chip.querySelector('.due-date-caption')?.remove();
  ...
  db.auth.getSession().then(({data}) => {
    if (data?.session?.user) loadClassStarts();
  });
