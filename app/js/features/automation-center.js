(() => {
    ...
    async function loadReadiness() {
        ...
        document.getElementById('automationReadiness').innerHTML = checks.map(check => `
            <div class="automation-check ${check.ok ? 'ok' : 'pending'}"><b>${check.ok ? '✓' : '!'}</b><div><strong>${safeText(check.title)}</strong><span>${safeText(check.detail)}</span></div></div>`).join('');
    }
    ...
    async function retryMessage(button) {
        ...
        try {
            ...
            if (error) throw error;
            if (typeof toast === 'function') toast(data?.status === 'duplicate' ? 'Reenvio já processado.' : 'Mensagem reenviada.');
            delete button.dataset.retryRequestId;
            await loadMessages();
        } catch (error) {
            const text = String(error?.message || '');
            if (typeof toast === 'function') toast(text.includes('503') ? 'Conecte a Meta para realizar o reenvio.' : 'Não foi possível reenviar agora.');
