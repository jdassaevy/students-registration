    ...
    const api = {
        ...
        forPayment(studentId, person, kind, installment = 0) {
            return api
                .items
                .filter(
                    item => item.student_id === studentId && item.person === person && item.kind === kind && Number(item.installment || 0) === Number(installment || 0)
                );
        },
        activeForPayment(studentId, person, kind, installment = 0) {
            return api
                .forPayment(studentId, person, kind, installment)
